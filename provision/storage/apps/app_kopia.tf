resource "kubernetes_secret_v1" "kopia" {
  ...
  data = {
    "repository.config" = "${file("${path.module}/templates/repository.config")}"
  }
}
    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          env {
            name  = "KOPIA_PASSWORD"
            value = data.sops_file.secrets.data["kopia_repository_password"] #"none"
          }
          env {
            name  = "TZ"
            value = "Etc/UTC"
          ...
          volume_mount {
            name       = "data"
            mount_path = "/apheon" # tech-debt tycho
            # read_only  = true
          }
          resources {
            requests = {
        ...
        volume {
          ...
          host_path {
            path = "/eros/backups"
            type = "DirectoryOrCreate" # Set the volume type to "Directory" (optional)
          }
        }
        volume {
          name = "data"
          host_path {
            path = "/home/apheon/"
            type = "Directory" # Set the volume type to "Directory" (optional)
          }
        }
        toleration {
          effect   = "NoSchedule"
